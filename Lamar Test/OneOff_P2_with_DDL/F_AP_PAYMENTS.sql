...
IF OBJECT_ID('Oracle_Reporting_P2.Oracle_Reporting_P2.svo.F_AP_PAYMENTS','U') IS NOT NULL
    DROP TABLE Oracle_Reporting_P2.Oracle_Reporting_P2.svo.F_AP_PAYMENTS;
GO

CREATE TABLE Oracle_Reporting_P2.Oracle_Reporting_P2.svo.F_AP_PAYMENTS
(
...
TRUNCATE TABLE Oracle_Reporting_P2.Oracle_Reporting_P2.svo.F_AP_PAYMENTS
GO

INSERT INTO Oracle_Reporting_P2.Oracle_Reporting_P2.svo.F_AP_PAYMENTS
(
...
SELECT 
 ...
 FROM [DW_BronzeSilver_PROD].[bzo].[AP_InvoiceHeaderExtractPVO] IH                       
LEFT JOIN [DW_BronzeSilver_PROD].[bzo].[AP_PaidDisbursementScheduleExtractPVO] D       ON  IH.ApInvoicesInvoiceId  = D.ApInvoicePaymentsAllInvoiceId

LEFT JOIN [DW_BronzeSilver_PROD].[bzo].[AP_DisbursementHeaderExtractPVO] H             ON H.ApChecksAllCheckId = D.ApInvoicePaymentsAllCheckId
LEFT JOIN [DW_BronzeSilver_PROD].[bzo].[AP_InvoicePaymentScheduleExtractPVO] S         ON S.ApPaymentSchedulesAllInvoiceId = D.ApInvoicePaymentsAllInvoiceId
                                                                        AND S.ApPaymentSchedulesAllPaymentNum = D.ApInvoicePaymentsAllPaymentNum
LEFT JOIN Oracle_Reporting_P2.svo.D_LEGAL_ENTITY      LE  ON LE.LEGAL_ENTITY_ID      = H.ApChecksAllLegalEntityId
LEFT JOIN Oracle_Reporting_P2.svo.D_VENDOR_SITE       V   ON V.VENDOR_SITE_ID        = H.ApChecksAllVendorSiteId
LEFT JOIN Oracle_Reporting_P2.svo.D_BUSINESS_UNIT     BU  ON BU.BUSINESS_UNIT_ID     = D.ApInvoicePaymentsAllOrgId
LEFT JOIN Oracle_Reporting_P2.svo.D_LEDGER            LDG ON LDG.LEDGER_ID           = D.ApInvoicePaymentsAllSetOfBooksId

LEFT JOIN stage.LINES_CODE_COMBO_LOOKUP C                  ON CAST(IH.ApInvoicesAcctsPayCodeCombinationId AS BIGINT) = C.CODE_COMBINATION_BK
LEFT JOIN Oracle_Reporting_P2.svo.D_ACCOUNT           DA  ON DA.ACCOUNT_ID            = C.ACCOUNT_ID
LEFT JOIN Oracle_Reporting_P2.svo.D_BUSINESS_OFFERING DBO ON DBO.BUSINESS_OFFERING_ID = C.BUSINESSOFFERING_ID
LEFT JOIN Oracle_Reporting_P2.svo.D_COMPANY           DCO ON DCO.COMPANY_ID           = C.COMPANY_ID
LEFT JOIN Oracle_Reporting_P2.svo.D_COST_CENTER       DCC ON DCC.COST_CENTER_ID       = C.COSTCENTER_ID
LEFT JOIN Oracle_Reporting_P2.svo.D_INDUSTRY          DI  ON DI.INDUSTRY_ID           = C.INDUSTRY_ID
LEFT JOIN Oracle_Reporting_P2.svo.D_INTERCOMPANY      DIC ON DIC.INTERCOMPANY_ID      = C.INTERCOMPANY_ID

LEFT JOIN Oracle_Reporting_P2.svo.D_CURRENCY INVC
       ON INVC.CURRENCY_ID =
...
LEFT JOIN Oracle_Reporting_P2.svo.D_CURRENCY PAYC
       ON PAYC.CURRENCY_ID =
          CONCAT(ISNULL(D.ApInvoicePaymentsAllPaymentCurrencyCode,'UNK'),
                 CONVERT(CHAR(8), CONVERT(CHAR(8), ISNULL(D.ApInvoicePaymentsAllExchangeDate,'0001-01-01'),112)),
                 'Corporate')
LEFT JOIN Oracle_Reporting_P2.svo.D_AP_INVOICE_HEADER DAPIH ON DAPIH.INVOICE_ID = IH.ApInvoicesInvoiceId
LEFT JOIN Oracle_Reporting_P2.svo.D_AP_DISBURSEMENT_HEADER DAPDH ON DAPDH.CHECK_ID = D.ApInvoicePaymentsAllCheckId
...
ON Oracle_Reporting_P2.svo.F_AP_PAYMENTS
(
    ACCOUNTING_DATE_SK,
    LEGAL_ENTITY_SK,
    BUSINESS_UNIT_SK,
