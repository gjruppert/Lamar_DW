...
BEGIN
    ...
    BEGIN TRY
        ...
        /* Optional: drop non-lookup indexes to speed inserts; recreate at end (or in CATCH) */
        IF @drop_non_lookup_indexes_during_load = 1
        BEGIN
            IF EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_F_GL_LINES_EFFDT_ACCTSK_COSK_ETC' AND object_id = OBJECT_ID('svo.F_GL_LINES'))
                DROP INDEX IX_F_GL_LINES_EFFDT_ACCTSK_COSK_ETC ON svo.F_GL_LINES;
            IF EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'UX_F_GL_LINES_GL_LINE_PK' AND object_id = OBJECT_ID('svo.F_GL_LINES'))
                DROP INDEX UX_F_GL_LINES_GL_LINE_PK ON svo.F_GL_LINES;
        ...
        BEGIN
            ...
            SELECT
                ...
                CAST(ISNULL(L.GLJELINESACCOUNTEDCR, 0) AS FLOAT),
                CAST(ISNULL(L.GLJELINESACCOUNTEDDR, 0) AS FLOAT),
                CASE L.GlJeLinesLedgerId WHEN '300000004574005' THEN 0 ELSE (CAST(ISNULL(L.GLJELINESACCOUNTEDDR, 0) AS FLOAT) - CAST(ISNULL(L.GLJELINESACCOUNTEDCR, 0) AS FLOAT)) * CAST(ISNULL(L.GLJELINESCURRENCYCONVERSIONRATE, 1) AS NUMERIC(7,4)) END,
            ...
            FROM DW_BronzeSilver_PROD.bzo.GL_JournalLineExtractPVO L
            INNER JOIN DW_BronzeSilver_PROD.bzo.GL_JournalHeaderExtractPVO H ON H.JEHEADERID = L.JEHEADERID
            ...
            SELECT @MaxWatermark = MAX(AddDateTime) FROM DW_BronzeSilver_PROD.bzo.GL_JournalLineExtractPVO;
        ...
        BEGIN
            ...
            SET @RowInserted = @@ROWCOUNT;
            SELECT @MaxWatermark = MAX(AddDateTime) FROM DW_BronzeSilver_PROD.bzo.GL_JournalLineExtractPVO WHERE AddDateTime > @LastWatermark;
        ...
        BEGIN
            IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_F_GL_LINES_EFFDT_ACCTSK_COSK_ETC' AND object_id = OBJECT_ID('svo.F_GL_LINES'))
                ...
                ON FG_SilverFact;
            IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'UX_F_GL_LINES_GL_LINE_PK' AND object_id = OBJECT_ID('svo.F_GL_LINES'))
                CREATE UNIQUE NONCLUSTERED INDEX UX_F_GL_LINES_GL_LINE_PK ON svo.F_GL_LINES(GL_LINE_PK) ON FG_SilverFact;
            PRINT 'usp_Load_F_GL_LINES: Recreated non-lookup indexes.';
        END
