...
BEGIN
    ...
    SELECT @TableBridgeID = TableBridgeID FROM meta.MedallionTableBridge WHERE targettable = N'AR_PartySiteExtractPVO';
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            (0, -1, -1, 'Unknown Party Site', 'UNK', -1, 'N', 'UNK', '0001-01-01', '9999-12-31', 'U', 'SYSTEM', 'SYSTEM', 'UNK',
             '0001-01-01', GETDATE(), @AsOfDate, @HighDate, @LoadDttm, @LoadDttm, 'Y');
    ...
    BEGIN CATCH
        ...
        IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'UX_D_PARTY_SITE_BK_CURR' AND object_id = OBJECT_ID('svo.D_PARTY_SITE'))
            CREATE UNIQUE NONCLUSTERED INDEX UX_D_PARTY_SITE_BK_CURR ON svo.D_PARTY_SITE(PARTY_SITE_ID) WHERE CURR_IND = 'Y' ON FG_SilverDim;
        ;THROW;
    END CATCH
END;
GO
