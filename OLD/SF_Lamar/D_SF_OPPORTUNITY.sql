/*=================================================================================================
File: 01_svo_D_SF_OPPORTUNITY.sql
Purpose: Create + plug + Type-1 load for Salesforce Opportunity dimension
Source:  bzo.Opportunity, bzo.OpportunityLineItem (optional totals rollup)
Grain:   1 row per Opportunity.Id
Notes:
- No stored procedures
- No MERGE
- Uses HASHDIFF to avoid pointless updates
- BZ_LOAD_DATE uses SystemModstamp (since no AddDateTime in your extract)
=================================================================================================*/
...
-- 1) DDL (drop + create)
-----------------------------------------------------------------------------------------
IF OBJECT_ID('svo.D_SF_OPPORTUNITY','U') IS NOT NULL
    DROP TABLE svo.D_SF_OPPORTUNITY;

CREATE TABLE svo.D_SF_OPPORTUNITY
(
    OPPORTUNITY_SK                              BIGINT IDENTITY(1,1) NOT NULL,
    ...
    CONSTRAINT PK_D_SF_OPPORTUNITY PRIMARY KEY CLUSTERED (OPPORTUNITY_SK) ON FG_SilverDim
) ON FG_SilverDim
  TEXTIMAGE_ON FG_SilverDim;

CREATE UNIQUE NONCLUSTERED INDEX UX_D_SF_OPPORTUNITY_BK
ON svo.D_SF_OPPORTUNITY (OPPORTUNITY_ID)
ON FG_SilverDim;
...
-----------------------------------------------------------------------------------------
-- 3) Source staging: roll up line totals and build #SrcOpportunity
-----------------------------------------------------------------------------------------
...
-- Update existing only when changed (skip plug row)
UPDATE d
SET
