...
/* =====================================================================
   ...
   Index:
     - Drops legacy unique index UX_D_INDUSTRY_ID if present (breaks SCD2 history)
     - Creates filtered unique index UX_D_INDUSTRY_ID_CURR for current rows only
   ===================================================================== */
...
(
    SELECT 1
    FROM sys.indexes
    WHERE name = 'UX_D_INDUSTRY_ID_CURR'
      AND object_id = OBJECT_ID('svo.D_INDUSTRY')
)
BEGIN
    CREATE UNIQUE NONCLUSTERED INDEX UX_D_INDUSTRY_ID_CURR
...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            INSERT INTO svo.D_INDUSTRY (INDUSTRY_SK, INDUSTRY_ID, INDUSTRY_LVL1_CODE, INDUSTRY_LVL1_DESC, INDUSTRY_LVL2_CODE, INDUSTRY_LVL2_DESC, INDUSTRY_LVL3_CODE, INDUSTRY_LVL3_DESC, INDUSTRY_DISTANCE, INDUSTRY_CATEGORY, INDUSTRY_ENABLED_FLAG, START_DATE_ACTIVE, END_DATE_ACTIVE, CREATED_BY, CREATION_DATE, BZ_LOAD_DATE, SV_LOAD_DATE, EFF_DATE, END_DATE, CRE_DATE, UDT_DATE, CURR_IND)
...
INNER JOIN src.bzo_GL_ValueSetValuesPVO lvl1 ON lvl1.VALUE = h1.DEP31PK1VALUE AND lvl1.ATTRIBUTECATEGORY = 'INDUSTRY LAMAR'
INNER JOIN src.bzo_GL_ValueSetValuesPVO lvl2 ON lvl2.VALUE = h1.DEP30PK1VALUE AND lvl2.ATTRIBUTECATEGORY = 'INDUSTRY LAMAR'
RIGHT JOIN src.bzo_GL_ValueSetValuesPVO lvl3 ON lvl3.VALUE = h1.DEP0PK1VALUE AND lvl3.ATTRIBUTECATEGORY = 'INDUSTRY LAMAR'
WHERE lvl3.ATTRIBUTECATEGORY = 'INDUSTRY LAMAR' AND lvl3.SUMMARYFLAG = 'N';
        ...
        UPDATE s
            SET s.SRC_HASH = HASHBYTES('SHA2_256', CONCAT(COALESCE(CONVERT(nvarchar(500), s.INDUSTRY_ID), N''), N'|', COALESCE(CONVERT(nvarchar(500), s.INDUSTRY_LVL1_CODE), N''), N'|', COALESCE(CONVERT(nvarchar(500), s.INDUSTRY_LVL1_DESC), N''), N'|', COALESCE(CONVERT(nvarchar(500), s.INDUSTRY_LVL2_CODE), N''), N'|', COALESCE(CONVERT(nvarchar(500), s.INDUSTRY_LVL2_DESC), N''), N'|', COALESCE(CONVERT(nvarchar(500), s.INDUSTRY_LVL3_CODE), N''), N'|', COALESCE(CONVERT(nvarchar(500), s.INDUSTRY_LVL3_DESC), N''), N'|', CONVERT(nvarchar(10), s.INDUSTRY_DISTANCE), N'|', COALESCE(CONVERT(nvarchar(500), s.INDUSTRY_CATEGORY), N''), N'|', COALESCE(CONVERT(nvarchar(500), s.INDUSTRY_ENABLED_FLAG), N''), N'|', CONVERT(nvarchar(10), s.START_DATE_ACTIVE, 120), N'|', CONVERT(nvarchar(10), s.END_DATE_ACTIVE, 120), N'|', COALESCE(CONVERT(nvarchar(500), s.CREATED_BY), N''), N'|', CONVERT(nvarchar(10), s.CREATION_DATE, 120)))
        FROM #src s;
        ...
        SELECT
              t.INDUSTRY_ID
            , HASHBYTES('SHA2_256', CONCAT(COALESCE(CONVERT(nvarchar(500), t.INDUSTRY_ID), N''), N'|', COALESCE(CONVERT(nvarchar(500), t.INDUSTRY_LVL1_CODE), N''), N'|', COALESCE(CONVERT(nvarchar(500), t.INDUSTRY_LVL1_DESC), N''), N'|', COALESCE(CONVERT(nvarchar(500), t.INDUSTRY_LVL2_CODE), N''), N'|', COALESCE(CONVERT(nvarchar(500), t.INDUSTRY_LVL2_DESC), N''), N'|', COALESCE(CONVERT(nvarchar(500), t.INDUSTRY_LVL3_CODE), N''), N'|', COALESCE(CONVERT(nvarchar(500), t.INDUSTRY_LVL3_DESC), N''), N'|', CONVERT(nvarchar(10), t.INDUSTRY_DISTANCE), N'|', COALESCE(CONVERT(nvarchar(500), t.INDUSTRY_CATEGORY), N''), N'|', COALESCE(CONVERT(nvarchar(500), t.INDUSTRY_ENABLED_FLAG), N''), N'|', CONVERT(nvarchar(10), t.START_DATE_ACTIVE, 120), N'|', CONVERT(nvarchar(10), t.END_DATE_ACTIVE, 120), N'|', COALESCE(CONVERT(nvarchar(500), t.CREATED_BY), N''), N'|', CONVERT(nvarchar(10), t.CREATION_DATE, 120))) AS TGT_HASH
        INTO #tgt
        ...
        INNER JOIN #delta d
            ON d.INDUSTRY_ID = tgt.INDUSTRY_ID
        WHERE tgt.CURR_IND = 'Y'
          AND tgt.END_DATE = @HighDate
          AND tgt.INDUSTRY_ID <> '-1';
        ...
        (
              INDUSTRY_ID, INDUSTRY_LVL1_CODE, INDUSTRY_LVL1_DESC, INDUSTRY_LVL2_CODE, INDUSTRY_LVL2_DESC, INDUSTRY_LVL3_CODE, INDUSTRY_LVL3_DESC, INDUSTRY_DISTANCE, INDUSTRY_CATEGORY, INDUSTRY_ENABLED_FLAG, START_DATE_ACTIVE, END_DATE_ACTIVE, CREATED_BY, CREATION_DATE, BZ_LOAD_DATE, SV_LOAD_DATE
            , EFF_DATE
            , END_DATE
            , CRE_DATE
            , UDT_DATE
        ...
        SELECT
              d.INDUSTRY_ID, d.INDUSTRY_LVL1_CODE, d.INDUSTRY_LVL1_DESC, d.INDUSTRY_LVL2_CODE, d.INDUSTRY_LVL2_DESC, d.INDUSTRY_LVL3_CODE, d.INDUSTRY_LVL3_DESC, d.INDUSTRY_DISTANCE, d.INDUSTRY_CATEGORY, d.INDUSTRY_ENABLED_FLAG, d.START_DATE_ACTIVE, d.END_DATE_ACTIVE, d.CREATED_BY, d.CREATION_DATE, d.BZ_LOAD_DATE, d.SV_LOAD_DATE
            , @AsOfDate
            , @HighDate
            , SYSDATETIME()
            , SYSDATETIME()
