...
BEGIN
    ...
    BEGIN TRY
        ...
        /* ===== Source (incremental + dedup by COST_CENTER_ID) ===== */
        ...
        (
            SELECT
                ...
                ROW_NUMBER() OVER
                (
                    PARTITION BY ISNULL(LTRIM(RTRIM(lvl4.VALUE)),'-1')
                    ORDER BY
                        (SELECT MAX(v.dt)
                         FROM (VALUES
                                (h1.AddDateTime),
            ...
            RIGHT JOIN bzo.GL_ValueSetValuesPVO lvl4
                ON lvl4.VALUE = h1.DEP0PK1VALUE
               AND lvl4.ATTRIBUTECATEGORY = 'CENTER LAMAR'
            WHERE
                lvl4.ATTRIBUTECATEGORY = 'CENTER LAMAR'
                ...
                (
                    ...
                    lvl4.AddDateTime > @LastWatermark
                )
        ) s
        WHERE s.rn = 1;
        ...
        INNER JOIN #src src
            ON src.COST_CENTER_ID = tgt.COST_CENTER_ID
        ...
        SELECT
            src.COST_CENTER_ID,
            src.COST_CENTER_LVL1_CODE, src.COST_CENTER_LVL1_DESC,
            src.COST_CENTER_LVL2_CODE, src.COST_CENTER_LVL2_DESC,
            src.COST_CENTER_LVL3_CODE, src.COST_CENTER_LVL3_DESC,
            src.COST_CENTER_LVL4_CODE, src.COST_CENTER_LVL4_DESC,
            src.COST_CENTER_DISTANCE,
            src.COST_CENTER_CATEGORY,
            src.COST_CENTER_ENABLED_FLAG,
            ...
            'Y'
        FROM #src src
        LEFT JOIN svo.D_COST_CENTER tgt
