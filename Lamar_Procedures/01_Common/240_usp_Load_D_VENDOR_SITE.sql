...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            VALUES
            (0, -1, -1, 'UNKNOWN', -1, 'UNK', 'UNKNOWN', 'UNKNOWN', 'UNKNOWN', 'UNKNOWN', 'UNKNOWN', 'UNKNOWN', 'UNKNOWN', '0000', 'UN', 'UNK', 'UNK', 'N', 'N', 'U', SYSDATETIME(), SYSDATETIME(), @AsOfDate, @HighDate, @LoadDttm, @LoadDttm, 'Y');
    ...
    BEGIN CATCH
        ...
        IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'UX_D_VENDOR_SITE_BK_CURR' AND object_id = OBJECT_ID('svo.D_VENDOR_SITE'))
            CREATE UNIQUE NONCLUSTERED INDEX UX_D_VENDOR_SITE_BK_CURR ON svo.D_VENDOR_SITE(VENDOR_SITE_ID) WHERE CURR_IND = 'Y' ON FG_SilverDim;
        ;THROW;
    END CATCH
END;
GO
