...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            (COST_CENTER_SK, COST_CENTER_ID, COST_CENTER_LVL1_CODE, COST_CENTER_LVL1_DESC, COST_CENTER_LVL2_CODE, COST_CENTER_LVL2_DESC,
             ...
             HISTORIC_COST_CENTER_LVL1_CODE, HISTORIC_COST_CENTER_LVL1_DESC, HISTORIC_COST_CENTER_LVL2_CODE, HISTORIC_COST_CENTER_LVL2_DESC,
             HISTORIC_COST_CENTER_LVL3_CODE, HISTORIC_COST_CENTER_LVL3_DESC,          HISTORIC_COST_CENTER_LVL4_CODE, HISTORIC_COST_CENTER_LVL4_DESC,
        ...
        SELECT
            ...
            COALESCE(prev.COST_CENTER_LVL1_CODE, src.COST_CENTER_LVL1_CODE), COALESCE(prev.COST_CENTER_LVL1_DESC, src.COST_CENTER_LVL1_DESC), COALESCE(prev.COST_CENTER_LVL2_CODE, src.COST_CENTER_LVL2_CODE), COALESCE(prev.COST_CENTER_LVL2_DESC, src.COST_CENTER_LVL2_DESC),
            COALESCE(prev.COST_CENTER_LVL3_CODE, src.COST_CENTER_LVL3_CODE), COALESCE(prev.COST_CENTER_LVL3_DESC, src.COST_CENTER_LVL3_DESC), COALESCE(prev.COST_CENTER_LVL4_CODE, src.COST_CENTER_LVL4_CODE), COALESCE(prev.COST_CENTER_LVL4_DESC, src.COST_CENTER_LVL4_DESC),
        ...
        LEFT JOIN svo.D_COST_CENTER prev ON prev.COST_CENTER_ID = src.COST_CENTER_ID AND prev.CURR_IND = 'N' AND prev.END_DATE = DATEADD(DAY, -1, @AsOfDate)
    ...
    BEGIN CATCH
        ...
        IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'UX_D_COST_CENTER_BK_CURR' AND object_id = OBJECT_ID('svo.D_COST_CENTER'))
            CREATE UNIQUE NONCLUSTERED INDEX UX_D_COST_CENTER_BK_CURR ON svo.D_COST_CENTER(COST_CENTER_ID) WHERE CURR_IND = 'Y' ON FG_SilverDim;
        ;THROW;
    END CATCH
END;
GO
