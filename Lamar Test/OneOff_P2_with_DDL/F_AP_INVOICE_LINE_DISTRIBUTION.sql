IF OBJECT_ID('Oracle_Reporting_P2.svo.F_AP_INVOICE_LINE_DISTRIBUTION','U') IS NOT NULL 
    DROP TABLE Oracle_Reporting_P2.svo.F_AP_INVOICE_LINE_DISTRIBUTION;
GO

CREATE TABLE Oracle_Reporting_P2.svo.F_AP_INVOICE_LINE_DISTRIBUTION
(
...
TRUNCATE TABLE Oracle_Reporting_P2.svo.F_AP_INVOICE_LINE_DISTRIBUTION;
GO

INSERT INTO Oracle_Reporting_P2.svo.F_AP_INVOICE_LINE_DISTRIBUTION (
...
LEFT JOIN DW_BronzeSilver_PROD.bzo.AP_InvoiceLineExtractPVO I         ON IH.ApInvoicesInvoiceId = I.ApInvoiceLinesAllInvoiceId 
LEFT JOIN DW_BronzeSilver_PROD.bzo.AP_InvoiceDistributionExtractPVO D ON I.ApInvoiceLinesAllInvoiceId = D.ApInvoiceDistributionsInvoiceId
...
LEFT JOIN Oracle_Reporting_P2.svo.D_AP_INVOICE_HEADER      AS H       ON H.INVOICE_ID             = IH.ApInvoicesInvoiceId
LEFT JOIN Oracle_Reporting_P2.svo.D_ACCOUNT                AS DA      ON DA.ACCOUNT_ID            = C.ACCOUNT_ID
LEFT JOIN Oracle_Reporting_P2.svo.D_BUSINESS_OFFERING      AS DBO     ON DBO.BUSINESS_OFFERING_ID = C.BUSINESSOFFERING_ID
LEFT JOIN Oracle_Reporting_P2.svo.D_COMPANY                AS DCO     ON DCO.COMPANY_ID           = C.COMPANY_ID
LEFT JOIN Oracle_Reporting_P2.svo.D_COST_CENTER            AS DCC     ON DCC.COST_CENTER_ID       = C.COSTCENTER_ID
LEFT JOIN Oracle_Reporting_P2.svo.D_INDUSTRY               AS DI      ON DI.INDUSTRY_ID           = C.INDUSTRY_ID
LEFT JOIN Oracle_Reporting_P2.svo.D_INTERCOMPANY           AS DIC     ON DIC.INTERCOMPANY_ID      = C.INTERCOMPANY_ID
LEFT JOIN Oracle_Reporting_P2.svo.D_LEGAL_ENTITY           AS LE      ON LE.LEGAL_ENTITY_ID       = IH.ApInvoicesLegalEntityId
LEFT JOIN Oracle_Reporting_P2.svo.D_BUSINESS_UNIT          AS BU      ON BU.BUSINESS_UNIT_ID      = IH.ApInvoicesOrgId
LEFT JOIN Oracle_Reporting_P2.svo.D_VENDOR_SITE            AS VS      ON VS.VENDOR_SITE_ID        = IH.ApInvoicesVendorSiteId
LEFT JOIN Oracle_Reporting_P2.svo.D_LEDGER                 AS LDG     ON LDG.LEDGER_ID            = IH.ApInvoicesSetOfBooksId
WHERE 1 = 1
;
