/*=================================================================================================
   svo.usp_Load_D_SF_OPPORTUNITY_T1
   Type-1 load for Salesforce Opportunity dimension.
   Source:  bzo.Opportunity, bzo.OpportunityLineItem (rollup for TOTAL_PRICE).
   Grain:   1 row per Opportunity.Id
   Prereq:  svo.D_SF_OPPORTUNITY must exist (run D_SF_OPPORTUNITY.sql DDL once, or create table first).
   Logging: etl.ETL_RUN
=================================================================================================*/
...
IF OBJECT_ID(N'svo.usp_Load_D_SF_OPPORTUNITY_T1', N'P') IS NOT NULL
    DROP PROCEDURE svo.usp_Load_D_SF_OPPORTUNITY_T1;
GO

CREATE PROCEDURE svo.usp_Load_D_SF_OPPORTUNITY_T1
(
    @AsOfDate DATE = NULL
)
AS
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            VALUES (0, N'-1', N'Unknown Opportunity', N'Unknown', 0, @AsOfDate, @AsOfDate);
        ...
        INTO #LineAgg
        FROM bzo.OpportunityLineItem oli
        ...
        SELECT
            ...
            LM_LOGO_INDUSTRY_CATEGORIES = o.LM_Logo_Industry_Categories__c,
            ...
            HASHDIFF = HASHBYTES('SHA2_256',
                CONCAT(
                    ...
                    COALESCE(o.LM_Logo_Industry_Categories__c, ''), '|',
            ...
            BZ_LOAD_DATE = CAST(o.SystemModstamp AS date),
            SV_LOAD_DATE = @AsOfDate
        INTO #SrcOpportunity
        FROM bzo.Opportunity o
        ...
        (
            ...
            LM_INVENTORY_ON_HOLD, LM_IS_TEAM_MATCH, LM_LAST_ACTIVITY_DATE, LM_LOGO_INDUSTRY_CATEGORIES, LM_NAICS_CODE,
        ...
        SELECT
            ...
            s.LM_INVENTORY_ON_HOLD, s.LM_IS_TEAM_MATCH, s.LM_LAST_ACTIVITY_DATE, s.LM_LOGO_INDUSTRY_CATEGORIES, s.LM_NAICS_CODE,
        ...
        SET
            ...
            d.LM_LOGO_INDUSTRY_CATEGORIES = s.LM_LOGO_INDUSTRY_CATEGORIES,
