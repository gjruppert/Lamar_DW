...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT
            ...
            ISNULL(CONVERT(INT, FORMAT(D.XlalinesAccountingDate, 'yyyyMMdd')), 10101),
        ...
        IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_F_SL_JOURNAL_DIST_APP_COMPANY' AND object_id = OBJECT_ID('svo.F_SL_JOURNAL_DISTRIBUTION'))
            CREATE NONCLUSTERED INDEX IX_F_SL_JOURNAL_DIST_APP_COMPANY ON svo.F_SL_JOURNAL_DISTRIBUTION(APPLICATION_ID, COMPANY_SK)
            INCLUDE (ACCOUNTING_DATE_SK, ACCOUNT_SK, BUSINESS_OFFERING_SK, COST_CENTER_SK, INDUSTRY_SK, INTERCOMPANY_SK, LEGAL_ENTITY_SK, BUSINESS_UNIT_SK, VENDOR_SITE_SK, LEDGER_SK, TRANSACTION_ENTITY_CODE, TRANSACTION_SOURCE_ID_INT1, TRANSACTION_NUMBER, EVENT_CLASS_CODE, EVENT_ID, EVENT_TYPE_CODE, SOURCE_DISTRIBUTION_TYPE, SOURCE_DIST_ID_NUM1, SOURCE_DIST_ID_NUM2, ACCOUNTING_CLASS_CODE, AE_LINE_NUM, CURRENCY_CODE, LINE_DESCRIPTION, ACCOUNTED_CR, ACCOUNTED_DR, ENTERED_CR, ENTERED_DR) ON FG_SilverFact;
        IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_F_SL_JOURNAL_DIST_APP_ACCOUNT' AND object_id = OBJECT_ID('svo.F_SL_JOURNAL_DISTRIBUTION'))
            CREATE NONCLUSTERED INDEX IX_F_SL_JOURNAL_DIST_APP_ACCOUNT ON svo.F_SL_JOURNAL_DISTRIBUTION(APPLICATION_ID, ACCOUNT_SK)
            INCLUDE (ACCOUNTING_DATE_SK, COMPANY_SK, BUSINESS_OFFERING_SK, COST_CENTER_SK, INDUSTRY_SK, INTERCOMPANY_SK, LEGAL_ENTITY_SK, BUSINESS_UNIT_SK, VENDOR_SITE_SK, LEDGER_SK, TRANSACTION_ENTITY_CODE, TRANSACTION_SOURCE_ID_INT1, TRANSACTION_NUMBER, EVENT_CLASS_CODE, EVENT_ID, EVENT_TYPE_CODE, SOURCE_DISTRIBUTION_TYPE, SOURCE_DIST_ID_NUM1, SOURCE_DIST_ID_NUM2, ACCOUNTING_CLASS_CODE, AE_LINE_NUM, CURRENCY_CODE, LINE_DESCRIPTION, ACCOUNTED_CR, ACCOUNTED_DR, ENTERED_CR, ENTERED_DR) ON FG_SilverFact;
