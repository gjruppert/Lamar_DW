...
BEGIN
    ...
    SELECT @TableBridgeID = TableBridgeID FROM meta.MedallionTableBridge WHERE targettable = N'AP_PaymentTermHeaderTranslationExtractPVO';
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            VALUES
            (0, '-1', 'Unknown', NULL, GETDATE(), @AsOfDate, @HighDate, @LoadDttm, @LoadDttm, 'Y');
        ...
        (
            SELECT
                ...
                CAST(src.AddDateTime AS DATETIME) AS BZ_LOAD_DATE,
    ...
    BEGIN CATCH
        ...
        IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'UX_D_PAYMENT_TERM_BK_CURR' AND object_id = OBJECT_ID('svo.D_PAYMENT_TERM'))
            CREATE UNIQUE NONCLUSTERED INDEX UX_D_PAYMENT_TERM_BK_CURR ON svo.D_PAYMENT_TERM(PAYMENT_TERM_ID) WHERE CURR_IND = 'Y' ON FG_SilverDim;
        ;THROW;
    END CATCH
END;
GO
