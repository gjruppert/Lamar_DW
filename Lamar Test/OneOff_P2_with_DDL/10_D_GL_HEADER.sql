...
TRUNCATE TABLE [svo].[D_GL_HEADER]
GO

IF NOT EXISTS (SELECT 1 FROM Oracle_Reporting_P2.svo.[D_GL_HEADER] WHERE GL_HEADER_SK = 0)
BEGIN
...
END;
    ...
    FROM DW_BronzeSilver_PROD.bzo.GL_JournalHeaderExtractPVO AS H
    LEFT JOIN Oracle_Reporting_P2.svo.D_LEDGER AS LGR ON LGR.LEDGER_ID = H.GLJEHEADERSLEDGERID
    LEFT JOIN [DW_BronzeSilver_PROD].[bzo].[GL_JournalCategoryExtractPVO] cat on H.GlJeHeadersJeCategory = cat.JournalCategoryJeCategoryName
	LEFT JOIN [DW_BronzeSilver_PROD].[bzo].[GL_JournalSourceExtractPVO] src on H.GlJeHeadersJeSource = src.JournalSourceJeSourceName;
