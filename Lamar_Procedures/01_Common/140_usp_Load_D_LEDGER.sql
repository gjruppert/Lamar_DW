...
BEGIN
    ...
    SELECT @TableBridgeID = TableBridgeID FROM meta.MedallionTableBridge WHERE targettable = N'GL_LedgerExtractPVO';
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            VALUES
            (0, -1, 'Unknown', 'NULL', 0, 'NULL', 'NULL', 0, 'NULL', 'NULL', GETDATE(), GETDATE(),
        ...
        (
            SELECT
                ...
                COALESCE(CAST(L.AddDateTime AS DATETIME), CAST('1900-01-01' AS DATETIME)) AS BZ_LOAD_DATE,
    ...
    BEGIN CATCH
        ...
        IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'UX_D_LEDGER_BK_CURR' AND object_id = OBJECT_ID('svo.D_LEDGER'))
            CREATE UNIQUE NONCLUSTERED INDEX UX_D_LEDGER_BK_CURR ON svo.D_LEDGER(LEDGER_ID) WHERE CURR_IND = 'Y' ON FG_SilverDim;
        ;THROW;
    END CATCH
END;
GO
