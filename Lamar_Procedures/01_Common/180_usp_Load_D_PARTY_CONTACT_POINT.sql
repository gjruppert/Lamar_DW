...
BEGIN
    ...
    SELECT @TableBridgeID = TableBridgeID FROM meta.MedallionTableBridge WHERE targettable = N'AR_PartyContactPointExtractPVO';
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            VALUES
            (0, 0, 0, 'UNKNOWN', 'UNKNOWN', 'UNKNOWN', 'N', '0001-01-01', 'SYSTEM', 'UNKNOWN', '0001-01-01', GETDATE(),
        ...
        (
            SELECT
                ...
                COALESCE(CAST(C.AddDateTime AS DATETIME), GETDATE()) AS BZ_LOAD_DATE,
    ...
    BEGIN CATCH
        ...
        IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'UX_D_PARTY_CONTACT_POINT_BK_CURR' AND object_id = OBJECT_ID('svo.D_PARTY_CONTACT_POINT'))
            CREATE UNIQUE NONCLUSTERED INDEX UX_D_PARTY_CONTACT_POINT_BK_CURR ON svo.D_PARTY_CONTACT_POINT(CONTACT_POINT_ID) WHERE CURR_IND = 'Y' ON FG_SilverDim;
        ;THROW;
    END CATCH
END;
GO
